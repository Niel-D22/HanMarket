...
/* SplashCursor — React Bits component, kept as the original (only typed for TS).
   ...
   - on unmount the frame loops stop and the listeners are removed. */
...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    if (!canvas) return;
    let disposed = false;
    const listeners: [EventTarget, string, any][] = [];
    const on = (target: EventTarget, type: string, fn: any, opts?: any) => {
    ...
    // safety: no WebGL / no renderable float textures -> skip the effect rather than crash the page
    if (!context || !context.ext.formatRGBA || !context.ext.formatRG || !context.ext.formatR) return;
    ...
    let frameId = 0;
    // The dye fully dissipates within a couple of seconds, so after that an idle frame draws nothing new.
    // Skipping those frames keeps the look identical while the page scrolls without a fluid sim running.
    ...
    function updateFrame() {
      if (disposed) return;
      const now = Date.now();
      if (pointers.some((p) => p.moved || p.down)) lastActive = now;
